...
//Função Djikstra
function findpath_Djikstra() {
    postMessage("running Djikstra");
    var graph,v,src;
    // Criando nossas listas abertas e fechadas
    var openList = new Array(v).fill(0);
    let dist=[];
    var closedList = new Array(config.grid_size);
    for (var i = 1; i < config.grid_size; i++) {
        closedList[i] = new Array(config.grid_size);
        dist.push([10000,-1]);
    ...
    dist[src][0] = 0;
    //inicializar valores closedList como 0
    for (var i = 0; i < config.grid_size -1; i++) {
        for (var j = 0; j < config.grid_size; j++) {
            let mn = -1;
            closedList[i][j] = 0;
            if(closedList[i][j]==-0){
